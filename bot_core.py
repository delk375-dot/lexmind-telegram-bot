...
def save_state(state: dict) -> None:
    """Зберігає оновлений стан індексів у файл (на Vercel — у /tmp)."""
    target = (Path("/tmp") / "state.json") if os.getenv("VERCEL") else STATE_FILE
...
# ─── build_application(): збірка Application для webhook (Vercel) ─────────────
# Без polling-lifecycle і без JobQueue (планові пости — на publish.yml).
# На Vercel радар НЕ реєструється (пише на read-only ФС).

def build_application():
    """Збирає Application і реєструє хендлери. Повертає app (не запускає)."""
    ...
    # Radar — лише поза Vercel (на serverless пише на read-only ФС)
...
async def sync_commands(app) -> None:
    """Синхронізує меню команд (2 scope). Викликається з локального bot.py."""
    ...
    logger.info("Команди синхронізовано: %d / %d (admin=%s)",
